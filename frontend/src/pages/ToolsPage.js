...
export default function ToolsPage() {
  ...
  const catColor = (cat) => {
    ...
    return colors[cat] || 'text-theme-faint bg-muted';
  };
    ...
    <div data-testid="tools-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-theme-faint mt-1">Configure tool policies and profiles</p>
        </div>
      ...
      <div className="bg-surface-card border border-subtle rounded-lg p-4">
        <h3 className="text-xs font-mono text-theme-faint mb-2 uppercase tracking-wider">Tool Groups</h3>
        <div className="flex flex-wrap gap-2">
          {TOOL_GROUPS.map(g => (
            <span key={g} className="text-xs font-mono px-2 py-1 rounded bg-surface-card border border-subtle text-theme-muted">{g}</span>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <Input data-testid="tool-search" value={search} onChange={e => setSearch(e.target.value)}
            className="pl-10 bg-surface-sunken border-subtle focus:border-orange-500 font-mono text-sm" placeholder="Search tools..." />
        </div>
        <Select value={filterCat} onValueChange={setFilterCat}>
          <SelectTrigger className="w-40 bg-surface-sunken border-subtle text-sm"><SelectValue placeholder="Category" /></SelectTrigger>
      ...
      {loading ? (
        ...
        <div className="bg-surface-card border border-subtle rounded-lg p-12 text-center">
          <Wrench className="w-12 h-12 text-theme-dimmed mx-auto mb-3" /><p className="text-theme-faint">No tools found</p>
        </div>
      ) : (
        Object.entries(grouped).map(([cat, items]) => (
          <div key={cat} className="space-y-2">
            ...
            <div className="bg-surface-card border border-subtle rounded-lg divide-y divide-subtle">
              {items.map(tool => (
                <div key={tool.id} data-testid={`tool-row-${tool.id}`} className="px-5 py-3 flex items-center justify-between hover:bg-muted/30 transition-colors">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className={`w-7 h-7 rounded flex items-center justify-center ${catColor(tool.category)}`}>
                      <Wrench className="w-3.5 h-3.5" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="text-sm font-mono text-theme-primary">{tool.tool_name}</h4>
                      <p className="text-xs text-theme-faint truncate">{tool.description}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 ml-4">
                    <Switch checked={tool.enabled} onCheckedChange={() => handleToggle(tool)} />
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(tool.id)} className="text-theme-faint hover:text-red-500 hover:bg-red-500/10">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-surface-card border-subtle max-w-md">
          ...
          <div className="space-y-4 mt-2">
            <div><Label className="text-theme-muted text-xs">Tool Name</Label><Input value={form.tool_name} onChange={e => setForm({...form, tool_name: e.target.value})} className="bg-surface-sunken border-subtle focus:border-orange-500 font-mono text-sm mt-1" /></div>
            <div><Label className="text-theme-muted text-xs">Description</Label><Input value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="bg-surface-sunken border-subtle focus:border-orange-500 text-sm mt-1" /></div>
            <div><Label className="text-theme-muted text-xs">Category</Label>
              <Select value={form.category} onValueChange={v => setForm({...form, category: v})}>
                <SelectTrigger className="bg-surface-sunken border-subtle text-sm mt-1"><SelectValue /></SelectTrigger>
                <SelectContent className="bg-surface-card border-subtle">{CATEGORIES.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>

...
const channelIcon = (type) => {
  ...
  return colors[type] || 'text-theme-muted bg-muted';
};
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label className="text-theme-muted text-xs">Channel Type</Label>
                <Select value={form.channel_type} onValueChange={v => setForm({...form, channel_type: v})}>
                  ...
                  <SelectContent className="bg-surface-card border-subtle max-h-60">{CHANNEL_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                </Select>
              </div>

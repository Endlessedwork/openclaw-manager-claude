    ...
    <div data-testid="skills-page" className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Manage agent skills and capabilities</p>
        </div>
      ...
      <div className="relative">
        ...
        <Input data-testid="skill-search" value={search} onChange={e => setSearch(e.target.value)}
          className="pl-10 bg-[#050505] border-zinc-800 focus:border-orange-500 font-mono text-sm" placeholder="Search skills..." />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[#0c0c0e] border border-zinc-800/60 rounded-lg divide-y divide-zinc-800/40">
          {filtered.length === 0 ? (
            <div className="p-12 text-center"><Zap className="w-12 h-12 text-zinc-700 mx-auto mb-3" /><p className="text-zinc-500">No skills found</p></div>
          ) : filtered.map(skill => (
            <div key={skill.id} data-testid={`skill-row-${skill.id}`} className="px-5 py-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
              <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center border ${skill.enabled ? 'bg-sky-500/10 border-sky-500/20' : 'bg-zinc-800 border-zinc-700'}`}>
                  <Zap className={`w-4 h-4 ${skill.enabled ? 'text-sky-500' : 'text-zinc-600'}`} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-medium text-zinc-200 font-mono">{skill.name}</h3>
                  ...
                  {skill.requires_env?.length > 0 && (
                    <div className="flex gap-1 mt-1">
                      {skill.requires_env.map(env => (
                        <span key={env} className="text-[10px] font-mono px-1 py-0 rounded bg-zinc-800 text-zinc-500">{env}</span>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2 ml-4">
                ...
                <Button variant="ghost" size="sm" onClick={() => openEdit(skill)} className="text-zinc-500 hover:text-orange-500 hover:bg-orange-500/10">
                  <Pencil className="w-3.5 h-3.5" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => handleDelete(skill.id)} className="text-zinc-500 hover:text-red-500 hover:bg-red-500/10">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-[#0c0c0e] border-zinc-800 max-w-xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
          ...
          <div className="space-y-4 mt-2">
            <div>
              ...
              <Input data-testid="skill-name-input" value={form.name} onChange={e => setForm({...form, name: e.target.value})}
                className="bg-[#050505] border-zinc-800 focus:border-orange-500 font-mono text-sm mt-1" placeholder="skill-name" />
            </div>
            <div>
              <Label className="text-zinc-400 text-xs">Description</Label>
              <Textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})}
                className="bg-[#050505] border-zinc-800 focus:border-orange-500 text-sm mt-1" rows={2} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label className="text-zinc-400 text-xs">Primary Env</Label>
                <Input value={form.primary_env} onChange={e => setForm({...form, primary_env: e.target.value})}
                  className="bg-[#050505] border-zinc-800 focus:border-orange-500 font-mono text-sm mt-1" placeholder="API_KEY_NAME" />
              </div>
            </div>
            <div>
              <Label className="text-zinc-400 text-xs">API Key</Label>
              <Input type="password" value={form.api_key} onChange={e => setForm({...form, api_key: e.target.value})}
                className="bg-[#050505] border-zinc-800 focus:border-orange-500 font-mono text-sm mt-1" placeholder="sk-..." />
            </div>
            <div>
              <Label className="text-zinc-400 text-xs">Required Env Vars (comma separated)</Label>
            ...
            <div>
              <Label className="text-zinc-400 text-xs">Required Bins (comma separated)</Label>
            ...
            <div>
              <Label className="text-zinc-400 text-xs">Homepage</Label>
              <Input value={form.homepage} onChange={e => setForm({...form, homepage: e.target.value})}
                className="bg-[#050505] border-zinc-800 focus:border-orange-500 font-mono text-sm mt-1" placeholder="https://..." />
            </div>
            <div className="flex items-center justify-between">
              <Label className="text-zinc-400 text-xs">Enabled</Label>
              <Switch checked={form.enabled} onCheckedChange={v => setForm({...form, enabled: v})} />
            </div>
            <div className="flex items-center justify-between">
              <Label className="text-zinc-400 text-xs">User Invocable</Label>
